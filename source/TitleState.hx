...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		titleBg.loadGraphic(Paths.getPath('images/titleBG.png', IMAGE));
		
		titleBg.antialiasing = ClientPrefs.globalAntialiasing;

		logoBl = new FlxSprite(titleJSON.titlex, titleJSON.titley);
		logoBl = new FlxSprite(330, 50);
        ...
        bgScroll = new FlxBackdrop(Paths.image('titleMenu/room_v2'), 0, 0, true, false);
		...
		bgScroll.setGraphicSize(Std.int(bgScroll.width * 1));
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					#if PSYCH_WATERMARKS
					createCoolText(['DeKart Engine by'], 15);
					#else
				...
				case 3:
					#if PSYCH_WATERMARKS
					addMoreText('Ragalik', 15);
					addMoreText('RaLmA', 15);
					addMoreText('Raindow', 15);
					#else
					addMoreText('present');
					#end
				// credTextShit.text += '\npresent...';
