...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['100k Dislikes!', 0.2], //From 0% to 19%
		['1000 Dislikes', 0.4], //From 20% to 39%
		['100 Dislikes', 0.5], //From 40% to 49%
		['10 Dislikes', 0.6], //From 50% to 59%
		['hated', 0.69], //From 60% to 68%
		['Disliked', 0.7], //69%
		['Nothing', 0.8], //From 70% to 79%
		['Liked', 0.9], //From 80% to 89%
		['Subscribed!', 1], //From 90% to 99%
		['Subscribed & Liked!!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	public static var isExtrasMode:Bool = true;
	...
	var dialogue:Array<String> = ['Text', 'lmao'];
	...
	override public function create()
	{
		...
		if(!modchartSprites.exists('blammedLightsBlack')) { //Creates blammed light black fade in case you didn't make your own
			blammedLightsBlack = new ModchartSprite(FlxG.width * -2.5, FlxG.height * -2.5);
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Likes: ' + songScore + ' | Dislikes: ' + songMisses + ' | Rank: ' + ratingName;
		} else {
			scoreTxt.text = 'Likes: ' + songScore + ' | Dislikes: ' + songMisses + ' | Rank: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
		}
		...
		var mult:Float = FlxMath.lerp(1, iconP1.scale.x, CoolUtil.boundTo(1 - (elapsed * 6), 0, 1));
		iconP1.scale.set(mult + 0.002, mult + 0.002);
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(1, iconP2.scale.x, CoolUtil.boundTo(1 - (elapsed * 6), 0, 1));
		iconP2.scale.set(mult - 0.002, mult + 0.002);
		iconP2.updateHitbox();
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			{
				trace('WENT BACK TO EXTRAS??');
				cancelMusicFadeTween();
				if(FlxTransitionableState.skipNextTransIn) {
					CustomFadeTransition.nextCamera = null;
				}
				MusicBeatState.switchState(new ExtrasSelectState());
				FlxG.sound.playMusic(Paths.music('freakyMenu'));
				changedDifficulty = false;
			}
			else
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		FlxTween.tween(rating, {alpha: 0, x: rating.x - 60} , 0.2, {
	...
	override function beatHit()
	{
		...
		FlxTween.tween(iconP1, {angle: -20}, 0.001, {
		...
		FlxTween.tween(iconP2, {angle: 20}, 0.001, {
			ease: FlxEase.quadOut,
			onComplete: function(twn:FlxTween)
			{
